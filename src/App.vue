...
export default {
  ...
  methods: {
    ...
    editTask(task) {this.update_mode = true;this.show = true;this.form.id = task.id;this.form.name = task.name;this.form.start = task.start;this.form.end = task.end;this.form.color = task.color;
    ...
    dragEnd(date) {
      ...
      if (dragIdLeft !== "") {
        let dragEvent = this.events.find((event) => event.id == dragIdLeft);
        dragEvent.start = date;
      ...
      if (dragIdRight !== "") {
        let dragEvent = this.events.find((event) => event.id == dragIdRight);
        dragEvent.end = date;
